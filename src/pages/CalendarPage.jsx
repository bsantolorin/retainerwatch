...
export default function CalendarPage() {
  ...
  const loadData = async () => {
    ...
    setEvents(effectiveView === 'client' ? eventList.filter(event => event.client_email === user.email) : eventList);
    setLoading(false);
  };
  ...
  return (
    <div>
      <PageHeader title={effectiveView === 'client' ? "My Calendar" : "Native Calendar"} subtitle={effectiveView === 'client' ? "Upcoming case dates and meetings shared with you." : "Manage attorney-specific deadlines, hearings, consultations, and meetings."} actions={effectiveView !== 'client' && <Button onClick={() => { setEditingEvent(null); setShowModal(true); }}><CalendarPlus className="w-4 h-4" /> Add Event</Button>} />
