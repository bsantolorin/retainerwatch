...
export default function TasksPage() {
  ...
  const loadData = async () => {
    ...
    setTasks(effectiveView === 'client' ? taskList.filter(task => task.client_visible) : taskList);
    setCases(caseList);
  ...
  return (
    <div>
      <PageHeader title={canManage ? "Tasks" : "My Tasks"} subtitle={canManage ? "Manage case work, assignments, deadlines, and client-visible milestones." : "Client-visible case milestones and tasks shared with you."} actions={canManage && <Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4" /> New Task</Button>} />
